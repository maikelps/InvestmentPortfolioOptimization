...
def multiple_points_co(p1, p2, positions = 3):
    """Implementation of multiple point crossover (inspired by single point crossover)

    Args:
        p1 (Individual): First parent for crossover.
        p2 (Individual): Second parent for crossover.
        positions : the number of placeholders to swap genes between parents.

    Returns:
        Individuals: Two offspring, resulting from the crossover.
    """
    co_points = []
    # We want to get exactly the number of positions in the parameter:
    while len(co_points) < positions:
        # Creating place holders, ordering and removing duplicates
        co_points = [randint(1, len(p1)-2) for _ in range(positions)]
        co_points.sort()
        co_points = set(co_points)

    for i, point in enumerate(co_points):
        if (i == 0):
            offspring1 = p1[:point] + p2[point:]
            offspring2 = p2[:point] + p1[point:]
        else:
            offspring1, offspring2 = offspring1[:point] + offspring2[point:] , offspring2[:point] + offspring1[point:]
...
if __name__ == '__main__':
    ...
    p2 = [9,8,7,6,0,4,3,2,1]

    a,b = multiple_points_co(p1, p2)